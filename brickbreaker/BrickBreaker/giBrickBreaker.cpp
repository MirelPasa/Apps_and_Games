...
std::shared_ptr<BrickBreakerGame>	gBrickBreaker=NULL;

string				gWndCaption = "Mirel Pasa-Brick Breaker";
//
    ...
    while ( msg.message != WM_QUIT )
    {
        ...
        else
        {
            ...
            if ( timeElapsed >= 0.01 )
            {
                ...
                gBrickBreaker->update ( ( float ) timeElapsed );
                // Draw every frame.
                gBrickBreaker->draw ( gBackBuffer->getDC (), ghSpriteDC );
...
LRESULT CALLBACK WndProc ( HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam )
{
    ...
    case WM_CREATE:
		...
		gBrickBreaker=std::make_shared<BrickBreakerGame>(ghAppInst,hWnd,gClientCenter);
    ...
    case WM_COMMAND:
		...
		case WM_KEYDOWN:
			...
			{
			case VK_ESCAPE:				
				if ( gBrickBreaker->mPaused==false)
				{	 
				gBrickBreaker->pause();
				}
				else 
				if ( gBrickBreaker->mPaused==true)
				{		
				gBrickBreaker->unpause();
				}
			break;
			}
			break;

...
HMENU				ghMainMenu = 0;
HDC					ghSpriteDC = 0;
std::shared_ptr<BackBuffer>gBackBuffer = NULL;
std::shared_ptr<FlappyBird>	gFlappyBird=NULL;

string				gWndCaption = "Mirel Pasa-Flappy Bird";
//
...
const int			gWindowHeight = gClientHeight + 39;
//
// * Function Prototypes ;
//
    ...
    while ( msg.message != WM_QUIT )
    {
        ...
        else
        {
            ...
            if ( timeElapsed >= 0.01 )
            {
                ...
                gFlappyBird->update ( ( float ) timeElapsed );
                // Draw every frame.
                gFlappyBird->draw ( gBackBuffer->getDC (), ghSpriteDC );
				//
				gFlappyBird->ProcessInput();
...
LRESULT CALLBACK WndProc ( HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam )
{
    ...
    case WM_CREATE:
		{
        // Create the brickbreaker game.
		gFlappyBird=std::make_shared<FlappyBird>(ghAppInst,hWnd,gClientCenter);
        // Create system memory DCs
        ghSpriteDC = CreateCompatibleDC ( 0 );
        // Create the backbuffer.
        gBackBuffer =std::make_shared<BackBuffer> ( hWnd, gClientWidth, gClientHeight );
		}
        return 0;
    case WM_COMMAND:
		...
		case WM_KEYDOWN:
			...
			{
			case VK_ESCAPE:				
				if ( gFlappyBird->mPaused==false)
				{	 
				gFlappyBird->pause();
				}
				else 
				if ( gFlappyBird->mPaused==true)
				{		
				gFlappyBird->unpause();
				}
			break;
	...
	case WM_LBUTTONDOWN:
			{
			if (!gFlappyBird->mPaused)
				if (gFlappyBird->nr_of_projectiles>0)
				{
				gFlappyBird->addProjectile();
				gFlappyBird->nr_of_projectiles--;
